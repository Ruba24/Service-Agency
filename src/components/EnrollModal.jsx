...
export default function EnrollModal({ course, onClose }) {
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      // Send request to your API to create Stripe checkout session
      const res = await fetch('/api/create-payment-intent', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          amount: Math.round((course.price || 49.99) * 100), // in cents
          metadata: {
            name: form.name,
            email: form.email,
            phone: form.phone,
      ...
      <div className="bg-white rounded-xl max-w-lg w-full p-6 relative">
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <button
            ...
            disabled={loading}
            className="px-4 py-2 rounded bg-[#B877F7] text-white hover:bg-[#A062D5]"
          >
            {loading ? 'Processing...' : `Pay $${course?.price ?? 49.99}`}
          </button>
        </div>
      </div>
    </div>
